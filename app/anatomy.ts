...
export interface Atlas {quality?:'full'|'optimized';version:string;sex?:'male'|'female';source?:string;scope?:string;parts:Part[];concepts:Concept[];chunks:{url:string;bytes:number;gzip?:string;gzipBytes?:number}[];materials?:Record<string,{color:number[];map?:string;opacity?:number;vertexColors?:boolean}>;triangles:number}
export type View = 'three-quarter'|'front'|'back'|'side'|'right'|'superior'|'inferior';
export interface SceneState {focus?:number;contextOpacity?:number;region?:string;peel?:number;depthHidden?:string[];hidden?:string[];labels?:boolean;camera?:number[];section?:{enabled:boolean;axis:'axial'|'sagittal'|'coronal';position:number;flip:boolean};skinOpacity?:number;inspectorOpen?:boolean;explode:number;visible:SystemId[];selected:string[];isolate:boolean;view:View;rotate:boolean;reset:number}
