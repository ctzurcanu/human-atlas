export type SystemId = 'skeletal'|'muscular'|'arterial'|'venous'|'nervous'|'digestive'|'respiratory'|'urinary'|'reproductive'|'lymphatic'|'endocrine'|'integumentary'|'connective'|'sensory'|'cardiac'|'pregnancy'|'attachments'|'regions'|'fascia'|'schematic';
...
export interface Part {suppressed?:boolean;tissue?:string;regions?:string[];depth?:number;sourceId?:string;groups?:string[];provenance?:{label:string;url:string;detail:string};material?:string;uvs?:number;id:string;name:string;conceptId:string;system:SystemId;chunk:number;positions:number;normals:number;indices:number;vertexCount:number;indexCount:number;bounds:[number[],number[]]}
export interface Concept {id:string;name:string;elements:string[]}
export interface Atlas {quality?:'full'|'optimized';version:string;sex?:'male'|'female';source?:string;scope?:string;parts:Part[];concepts:Concept[];chunks:{url:string;bytes:number;gzip?:string;gzipBytes?:number}[];materials?:Record<string,{color:number[];map?:string}>;triangles:number}
export type View = 'three-quarter'|'front'|'back'|'side'|'right'|'superior'|'inferior';
export interface SceneState {focus?:number;contextOpacity?:number;region?:string;peel?:number;hidden?:string[];labels?:boolean;camera?:number[];section?:{enabled:boolean;axis:'axial'|'sagittal'|'coronal';position:number;flip:boolean};skinOpacity?:number;inspectorOpen?:boolean;explode:number;visible:SystemId[];selected:string[];isolate:boolean;view:View;rotate:boolean;reset:number}
