...
interface Props {atlas:Atlas;state:SceneState;onSelect:(id:string,add?:boolean)=>void;onCamera?:(camera:number[])=>void;onProgress:(n:number)=>void;onError:(s:string)=>void}
export default function AnatomyScene({atlas,state,onSelect,onCamera,onProgress,onError}:Props){
 const host=useRef<HTMLDivElement>(null),latest=useRef(state),select=useRef(onSelect),cameraCallback=useRef(onCamera);
 latest.current=state;select.current=onSelect;cameraCallback.current=onCamera;
 useEffect(()=>{
  ...
  const scene=new T.Scene(),camera=new T.PerspectiveCamera(34,1,.0001,100),controls=new OrbitControls(camera,renderer.domElement);
  ...
  const materialFor=(system:string,ghost=false)=>{
   const m=new T.MeshStandardMaterial({color:system==='tendon'?'#d6c8b0':system==='cartilage'?'#afc0cb':SYSTEMS.find(s=>s.id===system)?.color??'#aebbb8',metalness:.03,roughness:.6,side:T.DoubleSide,transparent:ghost,opacity:1,depthWrite:!ghost,clippingPlanes:[clipPlane]});
   ...
   m.onBeforeCompile=shader=>{
    ...
    shader.fragmentShader=shader.fragmentShader.replace('#include <color_fragment>',`#include <color_fragment>
     ...
     float tissueAlpha = partSelected > 0.5 ? 1.0 : contextOpacity * ${system==='integumentary'?'skinOpacity':'1.0'};
     if (${ghost?'tissueAlpha >= 0.999 || tissueAlpha < 0.001':'tissueAlpha < 0.999'}) discard;
     diffuseColor.a = tissueAlpha;`);
   };materials.push(m);return m;
  };
  const mats=new Map([...SYSTEMS.map(s=>s.id),'tendon','cartilage'].map(id=>[id,materialFor(id)])),ghostMats=new Map([...SYSTEMS.map(s=>s.id),'tendon','cartilage'].map(id=>[id,materialFor(id,true)]));
  ...
  const loadChunk=async(ci:number)=>{
   ...
   atlas.parts.forEach((p,i)=>{
    ...
    const materialKey=p.tissue==='tendon'||p.tissue==='cartilage'?p.tissue:p.system;const list=groups.get(materialKey)??[];list.push(g);groups.set(materialKey,list);
   });
   groups.forEach((gs,system)=>{const geometry=mergeGeometries(gs,false);if(!geometry)throw new Error('Could not assemble anatomy geometry.');geometries.push(geometry);const mesh=new T.Mesh(geometry,mats.get(system as never));mesh.frustumCulled=false;mesh.renderOrder=2;scene.add(mesh);const ghost=new T.Mesh(geometry,ghostMats.get(system as never));ghost.frustumCulled=false;ghost.renderOrder=1;scene.add(ghost);});
  ...
  const fit=(view:string,extent=0)=>{
   ...
   controls.target.set(0,.85,0);const pivot=selectionCenter(atlas.parts,latest.current.selected,data);if(pivot)controls.target.copy(pivot);let fittedDistance=distance;if(latest.current.region&&latest.current.region!=='all'&&!pivot){const box=new T.Box3();atlas.parts.forEach((p,i)=>{if(p.system!=='integumentary'&&partVisible(p,latest.current))box.union(bounds[i]);});if(!box.isEmpty()){box.getCenter(controls.target);const size=box.getSize(new T.Vector3());const {w,h,left,right,top,bottom}=viewArea();camera.setViewOffset(w,h,w/2-(left+right)/2,h/2-(top+bottom)/2,w,h);fittedDistance=Math.max(size.y*h/Math.max(100,bottom-top),size.x*w/Math.max(100,right-left)/camera.aspect,size.z)/(2*Math.tan(T.MathUtils.degToRad(camera.fov/2)))*1.1;}}controls.maxDistance=Math.max(fittedDistance*1.25,.2);camera.position.copy(controls.target).addScaledVector(direction,fittedDistance);controls.update();dirty=true;
  };
  ...
  const down=(e:PointerEvent)=>{anchorSelection();hover.hidden=true;tap.down(e.pointerId,e.clientX,e.clientY,e.pointerType==='touch'?12:5);};
  ...
  const up=(e:PointerEvent)=>{
   ...
   let nearest=Infinity,found=-1;const hasSolid=(latest.current.skinOpacity??.1)<.95&&atlas.parts.some((p,i)=>p.system!=='integumentary'&&data[i*4+3]>.5);
   pickers.forEach((mesh,i)=>{if(!mesh||data[i*4+3]<.5||(!latest.current.selected.includes(atlas.parts[i].id)&&(contextUniform.value<.001||atlas.parts[i].system==='integumentary'&&skinUniform.value<.001))||(hasSolid&&atlas.parts[i].system==='integumentary'&&!latest.current.selected.includes(atlas.parts[i].id)))return;worldBox.copy(bounds[i]).translate(mesh.position);if(!raycaster.ray.intersectBox(worldBox,hitPoint))return;const hit=raycaster.intersectObject(mesh,false).find(h=>clipPlane.distanceToPoint(h.point)>=0);const selected=latest.current.selected.includes(atlas.parts[i].id),priority=selected&&contextUniform.value<1?1000:0;if(hit&&hit.distance-priority<nearest){nearest=hit.distance-priority;found=i;}});
  ...
  const drawLabels=(s:SceneState)=>{
   ...
   let row=0;for(const id of s.selected.slice(0,20)){const i=atlas.parts.findIndex(p=>p.id===id),mesh=pickers[i];if(i<0||!mesh)continue;
    ...
    make('line',{x1:String(x),y1:String(y),x2:String(labelX),y2:String(labelY),...(occluded?{'stroke-dasharray':'3 3'}:{})});make('circle',{cx:String(x),cy:String(y),r:'3'});const text=make('text',{x:String(labelX+5),y:String(labelY-5)});text.textContent=atlas.parts[i].name+(occluded?' (behind)':'');
   }
  };
  const clock=new T.Clock();let lastExtent=-1,lastCamera:number[]|undefined,lastFocus=0,hadSelection=false,lastAreaKey='';
  const animate=()=>{
   ...
   if(changed){const a=viewArea(),key=[a.w,a.h,a.left,a.right,a.top,a.bottom].join(':');if(key!==lastAreaKey){camera.setViewOffset(a.w,a.h,a.w/2-(a.left+a.right)/2,a.h/2-(a.top+a.bottom)/2,a.w,a.h);if(lastAreaKey&&s.focus)lastFocus=-1;lastAreaKey=key;dirty=true;}}

   if(changed){labelAnchors.clear();const box=new T.Box3();atlas.parts.forEach((p,i)=>{if(p.system!=='schematic'&&p.system!=='integumentary'&&partVisible(p,s))box.union(bounds[i]);});const section=s.section;if(section?.enabled&&!box.isEmpty()){const axis=section.axis==='axial'?1:section.axis==='sagittal'?0:2;const sign=section.flip?1:-1;clipPlane.normal.set(0,0,0).setComponent(axis,sign);clipPlane.constant=-sign*T.MathUtils.lerp(box.min.getComponent(axis),box.max.getComponent(axis),section.position);}else clipPlane.constant=10000;dirty=true;}
   ...
   const selectionKey=s.selected.join(',')+':'+s.reset;if(selectionKey!==lastSelection||moving&&s.selected.length>0){const pivot=selectionPivot();if(pivot){controls.target.copy(pivot);controls.update();}else if(hadSelection){fit(s.view,amount);}hadSelection=s.selected.length>0;lastSelection=selectionKey;dirty=true;}
