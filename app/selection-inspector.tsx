...
export default function SelectionInspector({titleRef,choice,selectedParts,partById,sex,scope,state,covering,depth,allowFadedSelection,onOpacity,onDepth,onAllowFadedSelection,onCenter,onIsolate,onHide,onChoosePart}:Props){
 ...
 return <>
  ...
  <div className="detail-scroll" key={choice.id}>
   <nav className="structure-breadcrumbs" aria-label="Anatomical path">{path.map((node,index)=><span className="breadcrumb-step" key={`${index}:${node.label}`}>{index>0&&<span className="breadcrumb-separator" aria-hidden="true">›</span>}<a href={index===0?`https://en.wikipedia.org/wiki/${node.search}`:index===path.length-1?wikipediaStructure(title):wikipediaSearch(node.search)} target="_blank" rel="noopener noreferrer" title={`Search Wikipedia for ${node.label}`}>{node.label}</a></span>)}</nav>
