...
import {buildAnatomyNodes,entryLabel,hierarchyEntries,type AnatomyEntry,type AnatomyNode} from './anatomy-hierarchy';
...
export default function DepthTree({atlas,state,setState,onChoose}:Props){
 ...
 const groups=useMemo(()=>{
  ...
  return DEPTH_LAYERS.map(layer=>{
   const entries=byLayer.get(layer.id)!;
   return {layer,parts:entries.flatMap(entry=>entry.parts),nodes:buildAnatomyNodes(entries,entry=>[entry.region,...entry.location.filter(name=>name.toLowerCase()!==layer.name.toLowerCase())],compareEntries)};
  });
 ...
 const toggleAll=()=>setState(current=>{
  ...
  const allOn=atlas.parts.every(part=>!part.suppressed&&!hiddenNow.has(part.id)&&!depthNow.has(depthLayerFor(part))&&visibleNow.has(part.system)&&(depthLayerFor(part)!=='skin'||(current.skinOpacity??1)>0));
  return {...current,visible:allOn?current.visible:[...new Set(atlas.parts.map(part=>part.system))],hidden:allOn?current.hidden:[],depthHidden:allOn?DEPTH_LAYERS.map(layer=>layer.id):[],skinOpacity:allOn?current.skinOpacity:(current.skinOpacity??0)>0?current.skinOpacity:.1,selected:[],isolate:false};
 });
 const toggleParts=(parts:Part[])=>setState(current=>{
  ...
  const allOn=parts.every(part=>!part.suppressed&&current.visible.includes(part.system)&&!hiddenNow.has(part.id)&&!depthNow.has(depthLayerFor(part))&&(depthLayerFor(part)!=='skin'||(current.skinOpacity??1)>0));
 ...
 const toggleLayer=(id:string,parts:Part[])=>setState(current=>{
  ...
  const allOn=parts.length>0&&parts.every(part=>!part.suppressed&&current.visible.includes(part.system)&&!(current.hidden??[]).includes(part.id)&&!depthNow.has(id)&&(id!=='skin'||(current.skinOpacity??1)>0));
  if(allOn){depthNow.add(id);return {...current,depthHidden:[...depthNow],selected:current.selected.filter(selected=>!parts.some(part=>part.id===selected)),isolate:false};}
  depthNow.delete(id);
  const ids=new Set(parts.map(part=>part.id)),hiddenNow=new Set(current.hidden??[]);
  const newlyEnabled=new Set(parts.filter(part=>!current.visible.includes(part.system)).map(part=>part.system));
  for(const part of atlas.parts)if(newlyEnabled.has(part.system)&&!ids.has(part.id))hiddenNow.add(part.id);
 ...
 const check=(label:string,parts:Part[],onClick:()=>void)=>{
  ...
  return <button type="button" className="tree-check" role="checkbox" aria-label={`Show ${label}`} aria-checked={status} data-state={status} disabled={!parts.length} onClick={onClick} title={enabled===parts.length?`Hide ${label}`:`Show ${label}`}>
 ...
 return <nav className="system-tree depth-tree" aria-label="Anatomical depth layers and structures">
  <div className="tree-row tree-root" style={{'--tree-depth':0} as CSSProperties}>
   ...
   {check('all anatomy',atlas.parts,toggleAll)}
  </div>
  {expanded.has('all')&&groups.map(({layer,parts,nodes},index)=>{
