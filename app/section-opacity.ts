...
export function sectionCapOpacity(part:Part,state:Pick<SceneState,'selected'|'contextOpacity'|'skinOpacity'>,sourceOpacity=1){
 const skin=isSkinPart(part)?state.skinOpacity??.1:1;
 const context=state.selected.includes(part.id)?1:state.contextOpacity??1;
