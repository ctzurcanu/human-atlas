...
export function partVisible(p:Part,s:SceneState){
 ...
 if(!isSurfaceSystem(p.system)&&s.visible.some(isSurfaceSystem)&&(s.skinOpacity??0)>=.999&&(s.explode??0)<.001)return false;
